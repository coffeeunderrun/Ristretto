...
{ Using heap manager from FPC embedded target. May replace with my own at some point.}
...
var
  initialheap : record end; external name '_heap_start';
  heapsize : PtrUInt; external name '_heap_size';

begin
  SetMemoryManager(MyMemoryManager);
  RegisterHeapBlock(@initialheap, PtrUInt(@heapsize));
end.
